    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: 118 }}>
      {data.map((d, i) => (
        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', height: '100%', gap: 4 }} title={`${fmt.n(d.tokens)} tokens/run · ${d.runs} runs`}>
          <div style={{ height: `${d.tokens / max * 100}%`, borderRadius: 3, background: i === data.length - 1 ? 'var(--cache)' : 'var(--accent)', opacity: i === data.length - 1 ? 1 : .16 + i / data.length * .5, transition: 'height .5s' }} />
        </div>
      ))}
    </div>
  );
          ...
          <div style={{ maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div className="list">
              ...
              <div className="row" style={{ cursor: 'default' }}>
                <div style={{ flex: 1 }}><div className="row-t">Theme</div><div className="row-s">Light and dark share one set of tokens.</div></div>
                <Seg value={appearance} onChange={setAppearance} options={[{ value: 'light', label: 'Light' }, { value: 'dark', label: 'Dark' }]} />
              </div>
              <div className="row" style={{ cursor: 'default' }}>
