...
export async function runsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: { id: string } }>('/runs/:id', { preHandler: [requireTenant] }, async (request, reply) => {
    ...
    const { rows: stepRows } = await pool.query(
      `SELECT sr.id, sr.step_id, sr.step_index, sr.status, sr.cache_hit, sr.selector_used, sr.duration_ms, sr.error_type, sr.failure_class, sr.healing_event_id, sr.screenshot_key, sr.content_hash, sr.target_hash, sr.user_verdict, sr.resolution_source, sr.similarity_score, sr.dom_candidates, sr.llm_picked_kaizen_id, sr.tokens_used, sr.captured_name, sr.captured_value, sr.frame_url, sr.created_at,
              ts.raw_text AS step_raw_text
       FROM step_results sr
       LEFT JOIN test_steps ts ON ts.id = sr.step_id
       WHERE sr.run_id = $1
       ORDER BY sr.step_index ASC NULLS LAST, sr.created_at ASC`,
      [request.params.id]
    );
