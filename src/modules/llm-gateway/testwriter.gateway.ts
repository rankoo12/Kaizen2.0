...
export class OpenAITestWriterGateway implements ITestWriterGateway {
  ...
  async generateScenario(input: WriteInput, tenantId: string): Promise<GeneratedScenario> {
    const system = [
      ...
      '{"action":"assert_text|assert_not_text","value":"<expected text>","target":{...}?}',
  ...
  async judgeScenarios(input: JudgeInput, tenantId: string): Promise<JudgeVerdict[]> {
    const system = [
      ...
      '   Pre-state test: "would every assertion already pass on the page as it existed BEFORE the key',
      '   action ran?" If yes -> FAIL. Also FAIL when the last state-changing action has no assertion after it.',
      '   GOOD: click Register -> verify the confirmation message is visible.',
      '   BAD:  navigate to /products -> verify the Products heading is visible (true before anything happened).',
