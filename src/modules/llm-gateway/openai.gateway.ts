...
{
  "action": "navigate" | "go_back" | "go_forward" | "reload" | "switch_tab" | "close_tab" | "click" | "click_random" | "double_click" | "right_click" | "hover" | "drag_and_drop" | "type" | "clear" | "select" | "check" | "uncheck" | "upload" | "assert_visible" | "assert_not_visible" | "assert_text" | "assert_not_text" | "assert_url" | "assert_title" | "assert_enabled" | "assert_disabled" | "assert_checked" | "assert_not_checked" | "assert_count" | "assert_attribute" | "wait" | "press_key" | "scroll",
...
- "assert_not_checked": verifying a checkbox or radio is NOT checked ("verify the newsletter box is unchecked"). Element in targetDescription.
...
CRITICAL — targetDescription rules:
- KEEP every qualifier that distinguishes the target from similar elements on the same page: the item/product name, the field's label, its section/group, or an ordinal (first/second/last). These are what pick the RIGHT element among lookalikes — dropping them makes two different targets collapse into one and resolve to the wrong element.
    * "the Add to cart button for the Sauce Labs Bike Light" → targetDescription: "Add to cart button for the Sauce Labs Bike Light"  (NOT "add to cart button")
    * "select Student from the Role dropdown" → targetDescription: "the Role dropdown"  (NOT "dropdown")
    * "click the second radio button" → targetDescription: "the second radio button"
- Strip only linguistic filler ("please", "the button that says", "go ahead and"). Never strip the identifying detail.
