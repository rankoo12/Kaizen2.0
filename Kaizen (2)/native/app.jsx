...
function App() {
  ...
  uEp(() => {
    const h = (e) => {
      ...
      else if (k === 'a' && e.shiftKey) { e.preventDefault(); const order = ['aperture', 'light', 'dark']; setT({ appearance: order[(order.indexOf(t.appearance) + 1) % 3] }); }
  ...
  const menus = [
    ...
    { label: 'View', items: [
      ...
      '-',
      { label: 'Next Appearance', key: '⇧⌘A', onClick: () => { const order = ['aperture', 'light', 'dark']; setT({ appearance: order[(order.indexOf(t.appearance) + 1) % 3] }); } },
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <TweaksPanel title="Kaizen">
        ...
        <TweakRadio label="Theme" value={t.appearance} options={['aperture', 'light', 'dark']} onChange={(v) => setT({ appearance: v })} />
