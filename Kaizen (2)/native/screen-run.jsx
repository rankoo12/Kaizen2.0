...
function RunScreen({ onBack, showToast, liveDefault }) {
  ...
  const restart = () => { healed.current = false; setHud(null); setLanded(0); setSel(null); setLive(true); setTab('steps'); };
  ...
  uEr(() => {
    if (done && live === false && landed === STEPS.length && liveDefault && hud === null) {
      const t = setTimeout(() => setHud(true), 340);
    ...
    <>
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        <div className="scroll" style={{ flex: 1, padding: '16px 20px 40px', minWidth: 0 }}>
          ...
          {tab === 'line' && window.LineView && (
            <window.LineView landed={landed} live={live} sel={sel} onSelect={setSel} />
          )}
      ...
      {hud && window.RunCompleteHUD && (
        <window.RunCompleteHUD status={status} steps={STEPS.length} cacheSteps={cacheSteps}
          tokens={tokens} prevTokens={RUN.prevTokens} ms={ms} prevMs={RUN.prevMs}
          learned={STEPS.filter((s) => s.heal).length + 1} onClose={() => setHud(false)} />
      )}
    </>
  );
}
