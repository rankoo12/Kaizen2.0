...
### Version 2.0 | Pure Learned Architecture | Lead: [Your Name] | PO: [PO Name]

*v2.0 Architectural Pivot: The specification has shifted from a Rule-Based/Composite parser to a Pure LearnedCompiler approach. We eliminated brittle Regex parsing in favor of exact-string hash caching backed entirely by the LLM. This provides zero-hardcoding linguistic flexibility with zero-latency cached test runs (the cache is pre-seeded with structural mappings to bypass cold starts).*
---
...
CREATE TABLE compiled_ast_cache (
  ...
  ast_json        JSONB NOT NULL,           -- e.g. {"action": "type", "value": "hello"}
  created_at      TIMESTAMPTZ DEFAULT now()
);
...
- `LearnedCompiler` — The sole compiler implementation (Version 2.0). Replaces regex engines with an LLM-backed exact-string hash dictionary (`compiled_ast_cache`). Translates any natural English into a structured Step AST; caches globally to maintain near-zero latency and eliminate hardcoding constraints. To prevent 100-step LLM cold starts, the cache is pre-seeded with standard structural patterns.
