...
/**
 * The full sign-in exchange: credentials → sessionToken → JWT pair → httpOnly cookies.
 *
 * Extracted so the demo-login route can perform exactly the same flow without a second
 * copy of the cookie handling. The important property is that tokens are set as httpOnly
 * cookies and never reach browser JS — that must not drift between the two entry points.
 */
export async function completeLogin(email: string, password: string): Promise<NextResponse> {
